...
async def __create_selector_agent(jwt_token: Optional[str] = None):
...
async def select(message: str, chat_history: list[ChatMessage] = [], jwt_token: Optional[str] = None):
    if not message:
        raise ValueError("No message provided")

    agent = await __create_selector_agent(jwt_token)
