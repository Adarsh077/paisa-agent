...
async def __create_sms_executor_agent(jwt_token: Optional[str] = None):
...
async def sms_execute(message: str, chat_history: list[ChatMessage] = [], jwt_token: Optional[str] = None):
    if not message:
        raise ValueError("No message provided")

    agent = await __create_sms_executor_agent(jwt_token)
